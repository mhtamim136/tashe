...
async function openSharedItemFromUrl() {
  ...
  if (!id || !type) return;
  if (type === 'folder') return loadFolder(id);
  if (type === 'file') {
    const item = { id, type: 'file', name: 'Shared file', mimeType: 'application/octet-stream' };
...
function showMenu(e, item) {
  const actions = [{ label: item.type === 'folder' ? 'Open' : 'Preview', icon: '↗️', run: () => openItem(item) }];
  if (item.type === 'file') actions.push({ label: 'Download', icon: '⬇️', run: () => downloadItem(item) });
  actions.push({ label: 'Share', icon: '🔗', run: () => shareItem(item) });
...
async function shareItem(item) { const data = await api.share(item); const url = new URL(window.TASHE_CONFIG.websiteBaseUrl); url.searchParams.set('share', data.token); url.searchParams.set('id', item.id); url.searchParams.set('type', item.type); navigator.clipboard?.writeText(url.toString()); ui.toast('Share link copied'); }
